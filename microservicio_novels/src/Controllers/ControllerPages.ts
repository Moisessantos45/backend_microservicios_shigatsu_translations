...
import { chapterData, novelData, volumenData } from "../Types/types";
...
const getNovel = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const dataNovel: novelData = novel.data() as novelData;
    const newNovel = { idNovel: novel.id, ...dataNovel };
...
const getVolumenesNovel = async (
  ...
  try {
    ...
    const newVolumene: volumenData[] = volumenes.docs.map((item) => {
      const { ...rest } = item.data();
      return { volumenId: item.id, ...rest } as volumenData;
    });
...
const getChaptersNovel = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const newChapters: chapterData[] = chapters.docs.map((item) => {
      ...
      return { capituloId: item.id, ...rest } as chapterData;
    });
